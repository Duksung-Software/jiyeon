...
class Gmarket2Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in range(1,101):
            ...
            if ( URL != [] ):
                ...
                yield scrapy.Request(url, callback= self.parse_page_content3)

            if ( URL == [] ):
                href = div.xpath('./div[1]/div[2]/div[1]/div[1]/span/a/@href')
                url = response.urljoin(href[0].extract())
                yield scrapy.Request(url, callback = self.parse_page_content4)
        

    def parse_page_content3(self, response):
        item = GMarket2Item()
    ...
    def parse_page_content4(self, response):
        item = GMarket2Item()
